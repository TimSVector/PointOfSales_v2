...
def parse_args():
    ...
    parser.add_argument('--vc_version',   help='Run specified test for specifc VC version',   action="store", dest="vc_version", default="")
...
def run_2018_post(args):
    ...
    for directory in directories:
        ...
        if args.run_all:
            ...
            print("2.1 Skipping " + directory + " test")
            continue
            
        print("2. run_version_tests", directory, "start time", datetime.now())
        os.chdir(directory)
        ...
        for coverage_type in coverage_types:
            ...
            for cargs in cli_args:
                ...
                print("2.2", directory, coverage_type, " ".join(callCmd))
...
def run_plugin(args):
    ...
    print("3. run_version_tests PluginTestRunner start time", datetime.now())

    if args.run_all or args.run_plgn:
        for vcd in [r'C:\VCAST\2022sp8', r'C:\VCAST\2023sp4']:
            print("3.1 run_version_tests PluginTestRunner " + vcd + " start time", datetime.now())
            os.environ['VECTORCAST_DIR'] = vcd
            p = subprocess.Popen(["PluginTestRunner.bat"], universal_newlines=True)
            p.wait()
    else:
        print("3.2 Skipping PluginTestRunner.bat")
...
if __name__ == '__main__':
    ...
    print("4. run_version_tests end time", datetime.now())
